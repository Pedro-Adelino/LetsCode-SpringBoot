...
// Classe para autorizacao (validar JWT token)
public class JWTAuthorizationFilter extends BasicAuthenticationFilter {
    ...
    // marcadas como excessao)
    @Override
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain chain)
            throws IOException, ServletException {
        String token = request.getHeader(SecurityConstants.HEADER_STRING);

        // Se não houver token, o filtro eh aplicado (nao autorizado, HTTP 403)
        ...
        // Caso tenha um valor (JWT foi validado), adiciona ao context da request e
        // continua o processamento (indo para a URL chamada em questao)
        UsernamePasswordAuthenticationToken authenticationToken = this.getAuth(token);
        SecurityContextHolder.getContext().setAuthentication(authenticationToken);
        chain.doFilter(request, response);
    }

    // getAuth verifica a autenticidade do token, atraves de "traduzir" o email e
    // roles que foram gerados para esse token. Caso não seja possivel, retorna null
